The script delta.py calculates the delta statistic from a file of gene trees in newick format. You must specify the triplet you are testing in the script. The script also estimates significance by creating 10,000 bootstrap pseudoreplicate tree files with replacement. The script outputs the delta statistic, counts of the different topologies, the standard deviation of delta, and a z score. 
...
The script delta.chisq.R conducts chi-square tests for the counts of discordant gene tree topologies and outputs a p-value.
